...
// Alternative PDF parsing - install with: npm install pdf-parse
...
export async function POST(request) {
  try {
    ...
    // For Azure Blob Storage URLs with SAS tokens, no additional auth header is needed
      ...
      try {
        const result = await generateText({ 
          ...
          messages: [{ 
            ...
            temperature: 0.7,
            maxTokens: 500,
            prompt:"you are AI assistant trained to answer questions based on the provided content(RAG).answer should be in paragraph format and should be concise and to the point. ",
          }] 
        });
...
// Fallback: If you want to keep using pdf.js, here's the original function
// But you'll need to fix the import issue first
/*
async function extractPdfToMarkdownWithPdfJs(pdfBuffer) {
  ...
  for (let i = 1; i <= pdf.numPages; i++) {
    ...
    textContent.items.forEach(item => {
      ...
      if (!str) return;
      if (prevY !== null && Math.abs(prevY - item.transform[5]) > 5) pageText += '\n';
      pageText += (pageText.endsWith(' ') ? '' : ' ') + str;
